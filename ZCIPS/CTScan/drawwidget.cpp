...
void DrawWidget::drawCurve(QPainter& _painter)
{
	...
	for(int i = 1; i != (*d_data).size(); ++i)
	{
		QLine line((i - 1) / d_xValuePerPixel, d_graphHeight - (*d_data)[i - 1] / d_yValuePerPixel,
			i / d_xValuePerPixel, d_graphHeight - (*d_data)[i - 1] / d_yValuePerPixel);
...
void DrawWidget::paintEvent(QPaintEvent * event)
{
	...
	int max = *itr / 10 * 10;
	d_graphWidth = width() - d_graphLeft - d_graphRightMargin;
	d_graphHeight = height() - d_graphTop - d_graphDownMargin;
	...
	for (int i = 0; i != 11; ++i)
	{
		QPoint point(i * d_xLineSpace + d_graphLeft, d_graphHeight - d_graphDownMargin);
		painter.drawText(point, QString("%1").arg(i * d_data->size() / 10));
	}

	painter.setViewport(d_graphLeft, d_graphTop, d_graphWidth, d_graphHeight);
	drawCurve(painter);
}
