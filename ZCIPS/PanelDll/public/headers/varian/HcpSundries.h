...
// supported function signatures for SOpenReceptorLink::FgCallbackPtr
typedef void(*FgCallbackFuncType0)();
typedef void(*FgCallbackFuncType1)(int errCode);

// allowed values for SOpenReceptorLink::FgCallbackFlag
#define	HCP_FG_CALLBACK_FLAG	0x0cbf // callback signature is FgCallbackFuncType0
#define HCP_FG_CLBCK_0			0x0cbf // callback signature is FgCallbackFuncType0
#define HCP_FG_CLBCK_1			0x1cbf // callback signature is FgCallbackFuncType1, called once only (per acquisition)
#define HCP_FG_CLBCK_2			0x2cbf // callback signature is FgCallbackFuncType1, repeatedly on each occurrence


#define HCP_CA_CLBCK_3			0x3cbf // this callback used by CA to determine whether to use/not use image

#define	HCP_FG_CANCEL_FLAG	1
#define	HCP_FG_NO_WIRED_FLAG	2
#define	HCP_FG_NO_WIRELESS_FLAG	4
...
// NOTE: also may be used in rad mode gain calibrations to allow the caller 
// to screen individual images as they are acquired and accept or reject.
enum {
HCP_CTRLTYPE_MIN = 46,
HCP_CTRLTYPE_OFST,
HCP_CTRLTYPE_GAIN,
...
HCP_CTRLTYPE_GR1,
HCP_CTRLTYPE_GRCAL,
HCP_CTRLTYPE_GIS = 100, // gain image select - in this case not dependent on usingCalControl
HCP_CTRLTYPE_MAX
};
...
typedef struct SCalCtrl
{
	...
	int				GisFlag;	// flag set to HCP_CA_CLBCK_3 when using GainImageSelect
	void*			GisPtr;		// pointer to callback function when using GainImageSelect 
	int				Reserved4;
	int				Reserved3;
	int				Reserved2;
	int				Reserved1;
...
// The data structure and function typedef below may be used to allow 
// the calling application to screen rad gain images ("GainImageSelect").
// When the GisPtr and GisFlag are set in a prior call to vip_cal_control then as
// each rad image is acquired the callback will be made to GisPtr(SGisClbk*)
// by the VCP to determine whether the image is to be used or no.
typedef struct SGisClbk
{
	int				StructSize;
	int				ImageByteSize;
	WORD*			ImagePtr;
...
typedef void  (* GisCallbackType)(SGisClbk* gisData);
...
{
	int				StructSize;	// Initialize to sizeof(SCheckLink)
...
typedef struct SDeviceInfo1
{
	...
	char			AdapterMac[MIN_STR]; // Returns the MAC address of the adapter
	char			AdapterIp[MIN_STR];  // Returns the IP address of the adapter
												// (if applicable)
	int				ReceptorType; // duplicates SSysInfo field
	int				Reserved3;
	int				Reserved2;
	int				Reserved1;
} SDeviceInfo1;
...
typedef struct SOpenReceptorLink
{
	...
	void*			FgCallbackPtr;		// pointer to a callback function
										//	HCP_FG_CALLBACK_FLAG OR... 
										//	HCP_FG_CLBCK_0 ==> [void FgCallbackPtr()]
										//	HCP_FG_CLBCK_1 ==> [void FgCallbackPtr(int errorCode)]										// which is called during acquisition
	volatile int	Flag;
	int				ConnectTimeout;		// Amount of time to attempt to connect in ms. If the receptor
										// cannot be found and the connection started in this time,
										// vip_open_receptor_link will return with an error
										// corresponding to the state the process was in when the
										// timeout expired. For example, if the timeout expired while
										// waiting for a wireless panel association,
										// HCP_WIFI_NO_ASSOCIATION will be returned. If 0, use the
										// default timeout behavior (which varies between receptor and
										// link type).
} SOpenReceptorLink;
...
HCP_U_QPIWF,     // WiFi link quality and settings
HCP_U_QPIBL,     // Battery level
HCP_U_QPIDIAGDATA_POLL,  // Raw diagnostic pixel data
HCP_U_QPI_MAX
};

// Codes for qpi.Cancellation
#define NO_CANCEL              (0)
#define CANCEL_SUCCEEDED       (1)  ///< A cancelled exposure frame is captured or if the  vip_io_enable(HS_CANCEL) call is used
#define CANCEL_FAILED          (2)  ///< Exposed frame is captured following cancellation
#define CANCEL_TIMEOUT_PREP    (3)	///< PREP is pressed down but timed out waiting for REQ
#define CANCEL_TRUNCATE_EXPOK  (4)	///< Cancellation is failed but EXP is truncated
...
#define NUM_PIX_WITH_DATA	140	// both hw and sw including signature (8)
#define DATA_START_LOC		8	// hw data start here
#define NUM_PIX_WITH_HW_DATA	96		
#define SW_DATA_START_LOC		104		
#define FW_ID_LOC	 10 // location of receptor FW version
...
typedef struct SDiagData
{
	...
	WORD            Shock;			// 72  (pixel 40)
	WORD            BatteryData[10];	// 74  (pixel 41..50)
	WORD            CsPanelType;		// 94  (pixel 51)
	WORD            Reserved[16];		// 96  (pixel 52..67)
...
typedef struct SQueryProgInfoWF	// uType = HCP_U_QPIWF
{
	int	StructSize;	// Initialize to sizeof(SQueryProgInfoWF)
	int	LinkQuality;	// in range [0, 94]
	int	CountryCode;	// ISO 3166-1 numeric
	int	Channel;
	int	Bandwidth;	// in MHz
	int	Reserved[3];
	char	IOBoxFirmware[32];
	char	ReceptorFirmware[32];
	int	Awake; // 0 if receptor is in low power mode, !=0 if receptor is awake
} SQueryProgInfoWF;

typedef struct SQueryProgInfoBL	// uType = HCP_U_QPIBL
{
	int	StructSize;	// Initialize to sizeof(SQueryProgInfoBL)
	int	FullCapacity;	// in mAh
	int	RemainingCapacity;
	int	Cycle;
	int	Health;		// in %
	int	Voltage;	// in mV
	int	Temperature;	// in 0.01 degree Celsius
	int	SerialNumber;
	int	ChargingCurrent;// in mA
	int	ManufactureYear;
...
typedef union  UQueryProgInfo
{
	...
	SDiagData           qpidiag;
	SQueryProgInfoWF    qpiwf;
	SQueryProgInfoBL    qpibl;
} UQueryProgInfo;
...
typedef struct SCallStatus
{
	...
	SDiagData*		DiagData;		// pointer to diag data - NULL if not available
	long			Progress;
	long			Limit;
} SCallStatus;
