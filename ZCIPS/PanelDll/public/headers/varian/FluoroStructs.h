...
typedef struct SFluoroModePrms
{
	int   StructSize;
	int   FrameX;
	int   FrameY;
...
typedef struct SSeqPrms
{
	...
	int   FastModeSwitch;		// dflt = 0, 1 indicates Fast Modes Switch is on
								// call to vip_fluoro_set_prms will fail if 
								// Fast Mode Switch not available for given panel FW
} SSeqPrms;
...
typedef struct SAcqPrms
{
	...
	int   Handswitch;			// internal use only - must be zero, used by WiFi FG
	int   FastModeSwitch;		// dflt = 0 --> off
...
// GRABBING,
// RECORDING
typedef struct SLivePrms
{
	int   StructSize;
	volatile int	NumFrames;
	volatile int	BufIndex; // "VcpId" - index to buffer currently most recent for display
	MVH(4,1) // padding with 64-bit compiler to match VS default alignment
	volatile void*	BufPtr;	// pointer to buffer currently most recent for display
	volatile int	VideoStatus;
	volatile int	ErrorCode;
	volatile int	RecId;	// frame ID created by receptor
	volatile int	GrbId;	// frame ID created by grabber (pleora)
	volatile int	DroppedFrm;  // indicates if this frame was dropped (1 dropped / 0 not dropped)
	volatile int	TotalDrops;  // total number of dropped frames
	volatile int	Reserved1;
	MVH(4,2) // padding with 64-bit compiler to match VS default alignment
} SLivePrms;

// not implemented currently
typedef struct SThreshSelect
{
	int   StructSize;
	MVH(4,1) // padding with 64-bit compiler to match VS default alignment
	void* TimerPtr;
